...
var App = exports.App = function (options) {
  ...
  ['config', 'log', 'exceptions'].forEach(function (plugin) {
    self.plugins[plugin] = self.plugins[plugin] || require('./plugins/' + plugin);
  });  
};
...
App.prototype.init = function (options, callback) {
  ...
  var self = this,
      core = ['config', 'log', 'exceptions'];
  
  function onComplete() {
    self.initialized['core'] = true;
    self._options = null;
    callback();
  }
